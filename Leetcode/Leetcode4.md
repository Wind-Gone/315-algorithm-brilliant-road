...
本题最直接的想法是合并数组然后找出中位数。对于数组的排序采用归并排序的方法是相对来说时间复杂度最好的方法。该方法的时间复杂度为O(m+n)。不过题目要求的是时间复杂度为O(log(m+n))的算法，考虑划分数组的方式。对于数组nums[1]和nums[2]，我们按照下面的方法分割数组，要求左边的最大值小于右边的最小值。具体的一些细节详见代码部分的注释。

## Code

```java
// 偏暴力的算法不再给出，就是合并两个有序数组 + 找中位数，没有思维难度
...
public class Main {
    public static void main(String[] args) {
        ...
        int[] nums1 = {1, 2};
        int[] nums2 = {3, 4};
        System.out.println(a.findMedianSortedArrays(nums1, nums2)); //2.5
    }
}
```
